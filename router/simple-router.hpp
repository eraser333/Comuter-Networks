...
public:
  ...
  // need to use in ArpCache, so place it public
  void sendIcmpPacket(const Buffer& packet, const std::string& inIface, uint8_t icmp_type, uint8_t icmp_code);
...
private:
  ...
  ///////////////////////////////////////////////////////////////
  // new function, try to decompose big function 'handlePacket'
  ///////////////////////////////////////////////////////////////
  /*
    pass ethHeader to record ans
    false if this ethernet_hdr is illegal
  */
  bool checkEthHeader(ethernet_hdr*& ethHeaderPtr, const Buffer& packet, const std::string& inIface);

  /*
    handle IPV4 packet
  */
  bool checkIPV4Header(ip_hdr*& ipv4HeaderPtr,const Buffer& packet, const std::string& inIface);
  void keepPacketForwarding(const Buffer& packet, const std::string& inIface, uint32_t dest_ip);

  /*
    handle and send ICMP packet
  */
  bool checkIcmpHeader(const Buffer& packet, icmp_hdr*& icmpHeaderPtr);
  void sendIcmpEchoReply(const Buffer& packet, const std::string& inIface);
  /*
    handle ARP packet
  */
  
  bool checkArpHeader(arp_hdr*& arpHeaderPtr,const Buffer& packet, const std::string& inIface);
  void handleArpRequest(arp_hdr*& arpHeaderPtr);
  void handleArpReply(arp_hdr*& arpHeaderPtr);
  //////////////////////////////////////////////////////////////////
};
