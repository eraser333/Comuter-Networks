...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  for (const RoutingTableEntry& tableEntry: m_entries) {
    // match and have longest prefix
    if (isMatched) {
      if (((tableEntry.mask & tableEntry.dest) == (tableEntry.mask & ip)) && tableEntry.mask > result.mask)
        result = tableEntry;
    }
    else {
      if ((tableEntry.mask & tableEntry.dest) == (tableEntry.mask & ip)){
        isMatched = true;
        result = tableEntry;
      }
    }
